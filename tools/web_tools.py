...
WEB_SEARCH_MATRIX_SCHEMA = {
    ...
    "description": (
        "Search across configured web providers and report the provider "
        "capability matrix. Returns fused results when query is supplied, plus "
        "provider availability, active search/extract/crawl providers, Firecrawl "
        "surface status, and env-var presence by variable name only. Use this "
        "before scheduled research source capture."
    ),
    "parameters": {
        "type": "object",
        "properties": {
