#!/usr/bin/env python3
# HADTO-PATCH: ctx.rs integration + security
"""
...
from hadto_patches import ctx as _ctx_runtime_patch
...
def _check_all_guards(command: str, env_type: str, container_config: dict | None = None) -> dict:
...
_task_env_overrides: Dict[str, Dict[str, Any]] = _ctx_runtime_patch._TASK_ENV_OVERRIDES
...
def register_task_env_overrides(task_id: str, overrides: Dict[str, Any]):
    ...
    """
    _ctx_runtime_patch.register_task_env_overrides(task_id, overrides)
...
def clear_task_env_overrides(task_id: str):
    ...
    """
    _ctx_runtime_patch.clear_task_env_overrides(task_id)


def get_task_env_overrides(task_id: Optional[str]) -> Dict[str, Any]:
    """Return task-local terminal overrides shared with ctx bindings."""
    return _ctx_runtime_patch.get_task_env_overrides(task_id)


def get_task_cwd(task_id: Optional[str], default: Optional[str] = None) -> Optional[str]:
    """Return the effective cwd for a task-aware terminal session."""
    return _ctx_runtime_patch.get_task_cwd(task_id, default=default)
...
def terminal_tool(
    ...
    try:
        ...
        cwd = get_task_cwd(effective_task_id, default=overrides.get("cwd") or config["cwd"])
        ...
        approval_note = None
        container_config_for_guards = None
        if env_type in ("docker", "singularity", "modal", "daytona"):
            container_config_for_guards = {
                "container_cpu": config.get("container_cpu", 1),
                "container_memory": config.get("container_memory", 5120),
                "container_disk": config.get("container_disk", 51200),
                "container_persistent": config.get("container_persistent", True),
        ...
        if not force:
            approval = _check_all_guards(command, env_type, container_config=container_config_for_guards)
