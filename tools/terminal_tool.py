...
def _split_docker_volume_spec(spec: str) -> Optional[tuple[str, str, str]]:
    """Return ``(host, container, mode)`` for a docker ``-v`` style spec."""
    ...
    # Windows drive paths look like C:\path:/container or C:/path:/container.
...
def _map_host_path_to_docker_volume(path: str, volumes: list[Any]) -> Optional[str]:
    """Map a host path into a configured Docker bind mount, when possible."""
    if not path:
        return None
    try:
    ...
    for raw_volume in volumes or []:
        ...
        if not (
            ...
            # Named Docker volumes do not map host paths.
            continue
        try:
            host_root = os.path.abspath(os.path.expanduser(host_path))
        except Exception:
            continue
        host_root_norm = host_root.rstrip(os.sep) or os.sep
        if candidate == host_root_norm:
            return container_path
        prefix = host_root_norm.rstrip(os.sep) + os.sep
        if candidate.startswith(prefix):
            rel = os.path.relpath(candidate, host_root_norm)
            rel_posix = rel.replace(os.sep, "/")
            return f"{container_path.rstrip('/')}/{rel_posix}"
    return None
...
def _sanitize_backend_cwd(
    ...
    *,
    ...
    """Return a cwd that is valid for the selected backend."""
    ...
    if env_type == "docker":
        ...
        if mapped:
            if mapped != candidate:
                logger.info(
                    "Mapped %s=%r into Docker bind mount cwd %r.",
                    source,
                    candidate,
                    mapped,
                )
    ...
    if env_type in {"modal", "docker", "singularity", "daytona", "vercel_sandbox"}:
        ...
        if (is_host_path or is_relative) and candidate != fallback_cwd:
            logger.info(
                "Ignoring %s=%r for %s backend because it is not available "
                "inside the sandbox. Using %r instead.",
                source,
                candidate,
                env_type,
                fallback_cwd,
