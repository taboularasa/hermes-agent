...
Hermes ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, HubSpot CRM, Home Assistant, and more.
...
| **Integrations** | `hubspot_crm`, `ha_*`, MCP server tools, `rl_*` | HubSpot CRM, Home Assistant, MCP, RL training, and other integrations. |
...
Common toolsets include `web`, `terminal`, `file`, `browser`, `vision`, `image_gen`, `moa`, `skills`, `tts`, `todo`, `memory`, `session_search`, `cronjob`, `code_execution`, `delegation`, `clarify`, `hubspot`, `homeassistant`, and `rl`.
