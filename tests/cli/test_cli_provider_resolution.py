...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    config_state = {"model": {"default": "", "provider": "custom", "base_url": ""}}
    monkeypatch.setattr("hermes_cli.config.load_config", lambda: config_state)
    monkeypatch.setattr("hermes_cli.config.save_config", lambda cfg: config_state.update(cfg))

    answers = iter([
        "http://localhost:8000",  # base URL
        "n",                      # do not auto-add /v1; let probe suggest/save it
        "",                       # accept detected single model
        "",                       # context length
        "",                       # display name
    ])
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("getpass.getpass", lambda _prompt="": "local-key")

    hermes_main._model_flow_custom({})
    output = capsys.readouterr().out

    assert "Saving the working base URL instead" in output
    assert config_state["model"]["base_url"] == "http://localhost:8000/v1"
    assert config_state["model"]["api_key"] == "local-key"
