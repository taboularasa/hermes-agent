...
class TestPreflightCompression:
    ...
    def test_preflight_compresses_oversized_history(self, agent):
        ...
        # Set a small threshold so the history is oversized, but leave enough
        # headroom for the compacted prompt framing added by the current
        # compressor implementation.
        agent.context_compressor.context_length = 2000
        agent.context_compressor.threshold_tokens = 400

        # Build a history that will be large enough to trigger preflight
        # (each message ~50 chars ≈ 13 tokens, 40 messages ≈ 520 tokens > 400 threshold)
        big_history = []
        for i in range(20):
