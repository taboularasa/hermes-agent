...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        # Keep gateway inactivity monitors informed during long synchronous
        # tools (a single Python/plugin tool has no polling loop of its own).
        tool_heartbeat_stop = None
        if not _execution_blocked:
            tool_heartbeat_stop = agent._start_tool_activity_heartbeat(
                function_name
            )
