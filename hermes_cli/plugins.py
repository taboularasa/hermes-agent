...
class PluginContext:
    ...
    def register_command(
        ...
        if not clean:
            message = "Plugin '%s' tried to register a command with an empty name."
            logger.warning(message, self.manifest.name)
            logging.getLogger().warning(message, self.manifest.name)
            return
        ...
        try:
            ...
            if resolve_command(clean) is not None:
                ...
                logger.warning(message, self.manifest.name, clean)
                logging.getLogger().warning(message, self.manifest.name, clean)
                return
        except Exception:
