...
def load_env() -> Dict[str, str]:
    ...
    Runtime secrets are loaded via Doppler by ``hermes_cli.env_loader`` and
    mirrored into ``os.environ``. This helper intentionally does not parse
    ``~/.hermes/.env`` in production. In non-strict/test mode
    (``HERMES_REQUIRE_DOPPLER=0``), we still support loading a local
    sanitized env file for compatibility with setup flows and tests.
    """
    require_doppler = (
        os.getenv("HERMES_REQUIRE_DOPPLER", "1").strip().lower()
        not in {"", "0", "false", "no", "off"}
    )
    try:
        from hadto_patches.env import load_runtime_env, _load_env_file_values

        env_vars = load_runtime_env(project_env=get_project_root() / ".env")
        if not require_doppler:
            env_path = get_env_path()
            file_env = _load_env_file_values(env_path)
            if env_path.exists():
                return file_env
        return env_vars
    except Exception:
        if require_doppler:
            raise
        try:
            from hadto_patches.env import _load_env_file_values
