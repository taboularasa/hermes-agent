...
OPTIONAL_ENV_VARS = {
    ...
    "TAVILY_API_KEY": {
        ...
        "tools": ["web_search", "web_search_matrix", "web_extract", "web_crawl"],
        "password": True,
        "category": "tool",
    },
    "SEARXNG_URL": {
...
TERMINAL_CONFIG_ENV_MAP = {
    ...
    # Hadto additions: per-session container isolation + Vercel sandbox runtime.
    # Kept in the canonical map so the config-set bridge (which upstream moved
    # onto TERMINAL_CONFIG_ENV_MAP) still mirrors them to .env like the fork's
    # former inline _config_to_env_sync dict did.
