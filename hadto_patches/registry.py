...
def _module_registers_tools(module_path: Path) -> bool:
    ...
    if any(_is_registry_register_call(stmt) for stmt in tree.body):
        return True

    # Sealed-domain compatibility wrappers delegate registration to a
    # ``hadto_patches.*`` module. Follow that import and inspect the real
    # implementation so built-in tool discovery still finds the wrapper.
    repo_root = module_path.resolve().parents[1]
    for stmt in tree.body:
        if not isinstance(stmt, ast.ImportFrom) or stmt.module != "hadto_patches":
            continue
        for alias in stmt.names:
            target = repo_root / "hadto_patches" / f"{alias.name}.py"
            if target.exists() and _module_registers_tools(target):
                return True
...
class ToolRegistry:
    ...
    def is_toolset_available(self, toolset: str) -> bool:
        ...
        checks = self._snapshot_toolset_checks()
        return self._evaluate_toolset_check(toolset, checks.get(toolset))

    def check_toolset_requirements(self) -> Dict[str, bool]:
        """Return ``{toolset: available_bool}`` for every toolset."""
        toolsets = self.get_registered_toolset_names()
        checks = self._snapshot_toolset_checks()
        return {
            toolset: self._evaluate_toolset_check(toolset, checks.get(toolset))
