...
class GatewayConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "quick_commands_allowed_users": self.quick_commands_allowed_users,
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            qc_allowed = yaml_cfg.get("quick_commands_allowed_users")
            if qc_allowed is not None:
                if isinstance(qc_allowed, list):
                    gw_data["quick_commands_allowed_users"] = qc_allowed
                else:
                    logger.warning(
                        "Ignoring invalid quick_commands_allowed_users in config.yaml "
                        "(expected list, got %s)",
                        type(qc_allowed).__name__,
                    )
