...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    with _jobs_lock():
        ...
        for i, job in enumerate(jobs):
            ...
            # Hadto: normalize the fork's required-tools / required-web-backends /
            # workspace-coordinator fields on the write path (create_job does the
            # same via _normalize_string_list / normalize_workspace_coordinator_config).
            for key in ("required_tools", "required_web_backends"):
                if key in updates:
