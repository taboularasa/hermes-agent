...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _normalize_chat_type(chat_type: Any) -> str:
        """Normalize telegram chat types across real enums, strings, and mocks."""
        candidates = (
        ...
        for candidate in candidates:
            ...
            for token in ("supergroup", "private", "channel", "group"):
                if text == token or token in text:
                    return token
        return str(chat_type).lower()

    @classmethod
    def _chat_type_is(cls, chat_type: Any, *expected: Any) -> bool:
        """Compare telegram chat types robustly across enum/string test stubs."""
        actual = cls._normalize_chat_type(chat_type)
        return any(actual == cls._normalize_chat_type(item) for item in expected)
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            chat_type = "dm"
            if self._chat_type_is(chat.type, ChatType.GROUP):
                chat_type = "group"
            elif self._chat_type_is(chat.type, ChatType.SUPERGROUP):
                chat_type = "group"
                if chat.is_forum:
                    chat_type = "forum"
            elif self._chat_type_is(chat.type, ChatType.CHANNEL):
                chat_type = "channel"
    ...
    def _build_message_event(self, message: Message, msg_type: MessageType) -> MessageEvent:
        ...
        # Determine chat type
        chat_type = "dm"
        if self._chat_type_is(chat.type, ChatType.GROUP, ChatType.SUPERGROUP):
            chat_type = "group"
        elif self._chat_type_is(chat.type, ChatType.CHANNEL):
            chat_type = "channel"
